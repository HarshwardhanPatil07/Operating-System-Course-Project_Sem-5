...
private:
    ...
    void WRITE()
    {
        ...
        // Add two blank lines before new content if file is not empty
        if (outfile.tellp() > 0)  // Check if there's already content in the file
        {
        ...
        if (RA != -1)
        {
            ...
            outfile << output;  // Write the job's output
        }

        SI = 0;
        
        outfile << endl;  // Add a single newline after each job's output for clarity
        outfile.close();
